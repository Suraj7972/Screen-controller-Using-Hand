...
while True:
    ...
    if landmarkList != []:
        ...
        # Update pointer coordinates based on thumb position
        ...
        if prev_x != 0 and prev_y != 0:
            ...
            if abs(dx) > abs(dy):
                if dx > 20 and two_finger_detected:
                    pyautogui.hotkey('ctrl', 'tab')  # Switch to next tab
                    two_finger_detected = False
                elif dx < -20 and two_finger_detected:
                    pyautogui.hotkey('ctrl', 'shift', 'tab')  # Switch to previous tab
                    two_finger_detected = False

        # Check if two fingers are detected for swipe gesture
